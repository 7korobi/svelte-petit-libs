...
class Block {
  ...
  constructor({
    ...
    config,
    block,
  }: BlockToolConstructorOptions<BlockData, {}>) {
    this.api = api
  ...
  renderSettings() {
    ...
    function refresh() {
      ...
      btns[1].classList.toggle(
        settingsButtonActive,
        "center" === this.data.align
      )
      btns[2].classList.toggle(
        settingsButtonActive,
...
export class HR {
  ...
  static conversionConfig = {
    import(html: string): HRData {
      const match = html.match(
        /<HR \/>|<HR class="(|footnote|stripe|blank)" \/>$/i
      )
      if (match) {
        return {
  ...
  constructor({
    ...
    config,
    block,
  }: BlockToolConstructorOptions<HRData, {}>) {
    this.api = api
    this.readOnly = readOnly
    this.data = {
      type: data.type || "",
