...
import { isAndroid, isIOS, isLegacy, __BROWSER__ } from "./device"

type SIZE_WITH_SCALE = [number, number, number]

const default_vp = { width: 1, height: 1, scale: 1 }
const vp = isLegacy ? default_vp : window.visualViewport || default_vp
...
function ViewFollowZoom() {
  ...
  if (
    ...
    ViewBox.isLandscape !== ZoomBox.isLandscape
  ) {
