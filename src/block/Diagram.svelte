<script context="module" lang="ts">
  import { url, style } from "../lib/config"
  const marker = {
    " ": "",
  ...
  function byLine(...args) {
    ...
    lines.forEach((o) => {
      ...
      if (
        ...
        16 * style.gap_size * style.gap_size
      ) {
        cvp = vp
        cwp = wp
      }
...
>
  <svg
    style="font-size: {0.75 * zoom}rem;"
    viewBox="{root.x} {root.y} {root.width} {root.height}"
  >
    <marker
      class="edgePath"
